...
export default function OrgSelect() {
  ...
  useEffect(() => {
    authApi.me()
      .then(({ user: u, organizations }) => {
        ...
        // With exactly one organization there is nothing to choose — go straight through.
        if ((organizations || []).length === 1) choose(organizations[0]);
      })
      .catch(e => {
        setOrgs([]);
  ...
  const choose = org => {
    selectedOrg.set({ id: org.id, name: org.name });
    navigate('/programs');
  };
    ...
    <div className="min-h-screen relative" style={{ background: '#f4f6fb' }}>
      ...
      <div className="relative max-w-5xl mx-auto px-8 py-12" style={{ zIndex: 1 }}>
        ...
        <div className="mb-6 animate-fade-up">
          ...
          <p className="text-sm text-gray-500 mt-0.5">Choose which organization you're working in.</p>
        </div>
