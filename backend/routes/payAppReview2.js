...
//
// A copy of routes/payAppReview.js writing to its own table (pay_app_reviews_2), so nothing tried
// here can corrupt a real review. The plumbing is identical and already works: upload, contract
// picker, the job that outlives the request, history, the report page, the marked-up PDF.
//
// What is missing is the review itself. Where the real module runs its engines, this calls
// reviewPayApp() in lib/payApp2Skill.js — one function, currently empty. That is the slot the skill
// goes into.
//
// Everything else is shared by import, which is what makes this start out working. Fork a lib into
// a *2 of its own the moment you want to change how it behaves; editing a shared one changes the
// live module too.
...
const { attachReadCheck } = require('../lib/payAppEngines');
...
function visibleReview(req) {
  const row = db.prepare('SELECT * FROM pay_app_reviews_2 WHERE id=?').get(req.params.id);
...
router.get('/projects', (req, res) => {
  res.json(db.prepare(`
    ...
    FROM projects p
    LEFT JOIN pay_app_reviews_2 r ON r.project_id = p.id
...
// The project filing cabinet, mounted rather than repeated. It used to be a copy of the block in
// routes/payAppReview.js, and the copy is how the live module lost it: holding that module at its
// 1-2 August behaviour took its documents endpoints with it, and the frontend calls those. One
// router now, mounted here and under /api/pay-app-review. See routes/projectDocuments.js.
const projectDocuments = require('./projectDocuments');
const { addDocument, DOCX_MIME } = projectDocuments;
router.use(projectDocuments);
...
router.get('/latest-for-project', (req, res) => {
  ...
  const row = project_id
    ? db.prepare(`
        ...
        FROM pay_app_reviews_2 WHERE project_id = ?
        ORDER BY application_number DESC, created_at DESC LIMIT 1
      `).get(project_id)
    : db.prepare(`
        SELECT id, application_number, period_to, extracted_data
        FROM pay_app_reviews_2 WHERE project_name = ?
        ORDER BY application_number DESC, created_at DESC LIMIT 1
      `).get(project_name);
  if (!row) return res.json(null);
  ...
  try {
    ...
    if (projectId) {
      const prior = db.prepare(`
        SELECT application_number, extracted_data FROM pay_app_reviews_2
    ...
    // THE SLOT. The real module runs eight rule engines here; the sandbox calls one function in
    // lib/payApp2Skill.js and nothing else. Until that function is implemented it returns null,
    // and this review is stored honestly as unchecked rather than as a clean result.
    ...
    try {
      engineResult = await reviewPayApp({
        ...
        deliveryMethod,
        files: { current: currentFile },
      });
      if (engineResult) attachReadCheck(engineResult, readCheck);
      else engineError = 'No review logic is in place yet — lib/payApp2Skill.js is still empty.';
    } catch (err) {
      console.error('[payapp2] the skill threw:', err.stack || err.message);
      engineError = err.message || 'The review could not be run on this application.';
    }
    ...
    // Defaulted rather than read straight through. The slot this module exists to fill will hold
    // half-finished work by definition, and a missing count is not a reason to throw away a review
    // whose reading has already been paid for — SQLite refuses to bind undefined, so it would.
    const criticalCount = engineResult?.stats?.critical ?? 0;
    const failCount = engineResult?.stats?.failed ?? 0;
...
router.get('/', (req, res) => {
  ...
  let sql = `SELECT id, project_name, application_number, period_to, contract_sum_to_date,
             ...
             FROM pay_app_reviews_2 WHERE ${scope.sql}`;
...
router.delete('/:id', async (req, res) => {
  const row = visibleReview(req);
  db.prepare('DELETE FROM pay_app_reviews_2 WHERE id=?').run(req.params.id);
